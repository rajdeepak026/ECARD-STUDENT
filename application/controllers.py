...
def user_dash(user_id):
    ...
    all_info = Info.query.all()
    return render_template("user_dash.html", this_user=this_user, all_info=all_info)
...
def card_details(card,user_id):
    ...
    if card == "aadhar":
        ...
        if request.method=="POST":
            ...
            info6 = Info(atr_name="pin", atr_value=pin, c_name=card, user_id=user_id)
            info7 = Info(atr_name="status", atr_value="requested", c_name=card, user_id=user_id)
        ...
        if request.method=="POST":
            ...
            db.session.add_all([info1, info2, info3, info4, info5, info7])
            db.session.commit()
            return render_template("user_dash.html", this_user=this_user)
